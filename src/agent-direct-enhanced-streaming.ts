...
import { SSEProgressCallbacks } from './sse/types';
...
/**
 * Enhanced agent with SSE streaming callbacks
 */
export async function runAgentWithToolsAndContextStreaming(
  prompt: string,
  ...
  callbacks?.onProgress?.('Initializing agent...');
  ...
  if (isBrowserAgent) {
    callbacks?.onProgress?.('Loading 32 Playwright tools...');
    ...
    try {
      ...
      callbacks?.onProgress?.(`Loaded ${playwrightTools.length} Playwright tools (specialized)`);
      console.log(`[Browser Agent] Loaded ${playwrightTools.length} Playwright tools`);
    } catch (e: any) {
    ...
    callbacks?.onProgress?.('Loading research tools from MCP Gateway...');
    ...
    callbacks?.onProgress?.(`Loaded ${allTools.length} research tools (Context7, Perplexity, BrightData)`);
    console.log(`[Research Agent] Loaded ${allTools.length} research tools`);
  }
  ...
  if (contextInfo && Object.keys(contextInfo).length > 0) {
    ...
    callbacks?.onProgress?.('Context from Claude Code added');
  }
  ...
  callbacks?.onProgress?.(`Starting agent loop with ${messages.length} messages...`);
  ...
  while (iterations < maxIterations) {
    ...
    callbacks?.onProgress?.(`Iteration ${iterations}/${maxIterations} - Calling Claude Sonnet 4.5...`);
    ...
    if (message.usage?.cache_read_input_tokens) {
      const cacheTokens = message.usage.cache_read_input_tokens;
      console.log(`[Cache] Read: ${cacheTokens} tokens (90% cost savings!)`);
      callbacks?.onCacheHit?.(cacheTokens);
    }
    ...
    if (textBlocks.length > 0 && iterations === 1) {
      // Only send thinking on first iteration to avoid spam
      const thinkingText = textBlocks.map((b: any) => b.text).join('\n').substring(0, 500);
      callbacks?.onThinking?.(thinkingText);
    }

    // Check if done
    if (message.stop_reason === 'end_turn') {
      callbacks?.onProgress?.('Agent completed successfully');
    ...
    if (message.stop_reason === 'tool_use') {
      ...
      callbacks?.onProgress?.(`Executing ${toolUses.length} tool call${toolUses.length > 1 ? 's' : ''}...`);
      ...
      const toolResults = await Promise.all(
        toolUses.map(async (toolUse: any) => {
          try {
            ...
            const toolResult = JSON.stringify(result.data.result.content || result.data.result);
            ...
            callbacks?.onToolResult?.(toolUse.name, null, error.message);
      ...
      callbacks?.onProgress?.('Response truncated due to length');
